...
class OrderFacadeTest {
    ...
    @Test
    @DisplayName("주문 결제 시 회원 단위 분산락과 idempotency key를 함께 적용한다")
    void orderDelegatesToServiceWithDistributedLock() {
        ...
        given(orderService.order(1L, 10L, "order-1")).willReturn(response);

        OrderCreateResponse result = orderFacade.order(1L, 10L, "order-1");

        assertThat(result).isEqualTo(response);
        verify(orderService).order(1L, 10L, "order-1");
    }
}
